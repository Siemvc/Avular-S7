...
// ====== CAN Setup (Teensy 4.1, CAN1 pins: TX=22, RX=23) ======
...
bool requestAndWait(uint8_t dataId,uint8_t bmsAddr,CAN_message_t &rx,uint32_t timeout_ms=120){
  ...
  CAN_message_t tx; memset(&tx,0,sizeof(tx));
  tx.id = reqId; tx.flags.extended = 1; tx.len = 8;
  CanBus.write(tx);
  uint32_t start = millis();
  while (millis()-start < timeout_ms){
    if (CanBus.read(rx)){
      if (rx.flags.extended && rx.len==8 && rx.id==respId) return true;
    }
  }
  return false;
}
...
bool readMosCap(MosCap &o,uint8_t addr){
  ...
  o.state = rx.buf[0]; o.chgMOS = rx.buf[1]; o.dsgMOS = rx.buf[2];
  return true;
}
...
void setup(){
  Serial.begin(115200);
  delay(300);
  Serial.println("Teensy 4.1 Dual Daly BMS + micro_ros_platformio (serial)");

  // CAN @ 250k (Daly default per doc variants)
  CanBus.begin();
  CanBus.setBaudRate(DALY_CAN_500K?500000:250000);

  // micro-ROS node & publishers
  uros_init(115200);
}
...
void loop(){
  ...
  // Optional: provide cell voltages & temps when you add multi-frame parsers
  std::vector<uint16_t> cells1, cells2;
  std::vector<int8_t>   temps1, temps2;

  // Publish to ROS 2 (two topics)
